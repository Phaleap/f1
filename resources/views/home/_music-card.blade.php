<div id="music-card">
    <div class="img-wrap">
        <img id="hans-img" src="{{ asset('build/assets/hans.jpg') }}" alt="Hans Zimmer">
        <div class="music-eq">
            <span></span><span></span><span></span><span></span><span></span>
        </div>
    </div>
    <div id="music-info">
        <span id="music-label">Now Playing</span>
        <span id="music-name">Hans Zimmer</span>
        <span id="music-track">Two Steps From Hell</span>
        <div class="music-progress">
            <div class="music-progress-fill" id="progress-fill"></div>
        </div>
    </div>
    <div class="music-controls">
        <button id="play-pause-btn" onclick="togglePlayPause()" title="Play / Pause">
            <!-- Play icon -->
            <svg id="icon-play" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" style="display:none">
                <path d="M8 5v14l11-7z"/>
            </svg>
            <!-- Pause icon -->
            <svg id="icon-pause" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/>
            </svg>
        </button>
        <button id="mute-btn" onclick="toggleMute()" title="Toggle Audio">
            <!-- Unmute icon -->
            <svg id="icon-unmute" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3A4.5 4.5 0 0 0 14 7.97v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/>
            </svg>
            <!-- Mute icon -->
            <svg id="icon-mute" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" style="display:none">
                <path d="M16.5 12A4.5 4.5 0 0 0 14 7.97v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51A8.796 8.796 0 0 0 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06A8.99 8.99 0 0 0 17.73 19L19 20.27 20.27 19 5.27 4 4.27 3zM12 4L9.91 6.09 12 8.18V4z"/>
            </svg>
        </button>
    </div>
</div>